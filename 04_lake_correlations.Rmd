...
ggplot(distances %>% na.omit(), aes(x = dist, y = rho)) +
...
lakeCombsFull <- combn(lake.summaries$name, 2) %>%
  t() %>% as_tibble() %>% left_join(lake.summaries %>% rename(V1 = name)) %>%
  left_join(lake.summaries %>% rename(V2 = name, region2 = region)) %>%
  filter(region == region2) %>%
  mutate()
