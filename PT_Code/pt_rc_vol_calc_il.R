...
vol_files = list.files(vol_dir, pattern='L2.csv')

#make a level directory
hgt_dir = "/Users/rinab14/Documents/gradschool/lab/pressure_transducer_data/illinois_logger_data/il_cleaned_up/"

#list all of the level files
hgt_files = list.files(hgt_dir, pattern='2.csv')
...
for (jnd in 1:length(vol_files)) {
  ...
  toRead2 = paste("/Users/rinab14/Documents/gradschool/lab/pressure_transducer_data/illinois_logger_data/il_cleaned_up/",hgt_files[jnd], sep='')
  lk22 = read.csv(toRead2, header = TRUE, sep = ",", quote =  "")
  
  # these line will find associated height file to current area file
  pattern = vol_files[jnd]
  hgt_ind = which(hgt_files == pattern)
  
  hV = na.omit(lk22$LEVEL)
  #dates
  if (hgt_files[hgt_ind] == "GAL2.csv") {
    hd = as.Date(substring(lk22$X.Date, 2), "%m/%d/%Y")
  }else if (hgt_files[hgt_ind] == "HAL2.csv") {
    hd = as.Date(substring(lk22$X.Date, 2), "%m/%d/%Y")
  }else if (hgt_files[hgt_ind] == "MCL2.csv") {
    hd = as.Date(substring(lk22$X.Date, 2), "%m/%d/%Y")
  }else if (hgt_files[hgt_ind] == "MPL2.csv") {
    hd = as.Date(substring(lk22$X.Date, 2), "%m/%d/%Y")
  }else if (hgt_files[hgt_ind] == "TML2.csv") {
    hd = as.Date(substring(lk22$X.Date, 2), "%m/%d/%Y")
  }else if (hgt_files[hgt_ind] == "YSL2.csv") {
    hd = as.Date(substring(lk22$X.Date, 2), "%m/%d/%Y")
  }else{
    hd = as.Date(lk22$Date, "%m/%d/%Y")
  }
  #}
  hdaaa = na.omit(hd)
