...
#WA
vol_files = list.files(vol_dir, pattern='W2.csv')
...
#WA
hgt_files = list.files(hgt_dir, pattern='W2.csv')
...
for (jnd in 1:length(vol_files)) {
  ...
  hd = as.vector(lk22$X.Date)
  hdaa = as.Date(substring(lk22$X.Date, 2), "%m/%d/%Y")
  hdaaa = na.omit(hdaa)
